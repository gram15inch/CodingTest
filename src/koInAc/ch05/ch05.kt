...
// SAM 생성자를 사용해 listener 인스턴스 재사용 하기
    ...
    val btn1= Button05().setOnclickListener(listener)
    val btn2= Button05().setOnclickListener(listener)
}
...
// with 와 식을 본문으로 하는 함수를 활용해 알파벳 만들기
fun alphabetWithReturn() = with(StringBuilder()){ // 식이 본문
    for(letter in 'A'..'Z')
        this.append(letter)
    append("\nNOew I know the alphabet!")
    toString()
...
class TextView(context: String){
    var text =""
    var textSize =0.0
}
// apply 를 TextView 초기화에 사용하기
fun createViwWithCustomAttributes(context:String) = TextView(context).apply {
    text = "Sample Text"
    textSize = 20.0
}

// buildString 으로 알파벳 만들기
fun alphabetBuildString() = buildString{ // StringBuilder 를 활용해 String 을 만들떄 사용
    for(letter in 'A'..'Z')
        this.append(letter)
    append("\nNOew I know the alphabet!")
} // toString 이 없음 :: 자동 String 반환